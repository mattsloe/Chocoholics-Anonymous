...
void Service_Ledger::generate_APR() { //error with provider_directory being referenced (line 48)
...
void Service_Ledger::generate_EFT() {
    ...
    for (auto it = ledger.begin(); it != ledger.end(); it++) { //provider directory undefined
//
//        sid = it->first;
//
//        if (providers.find(sid) == providers.end()) continue;
